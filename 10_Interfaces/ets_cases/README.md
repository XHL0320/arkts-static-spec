# 10 Interfaces — ets_cases 目录说明

## 目录结构

```
ets_cases/
├── 10.1_Interface_Declarations/        [4 用例]  接口声明
│   ├── compile-pass/                   [4]  基础/泛型/extends 接口
│   ├── compile-fail/                   [0]
│   └── runtime/                        [0]
│
├── 10.2_Superinterfaces_Subinterfaces/ [7 用例]  超接口与子接口
│   ├── compile-pass/                   [3]  单/多/多层继承
│   ├── compile-fail/                   [4]  非接口、循环、自继承
│   └── runtime/                        [0]
│
├── 10.3_Interface_Members/             [5 用例]  接口成员
│   ├── compile-pass/                   [3]  含 ISS-10-01 (FAIL 命名 GAP)
│   ├── compile-fail/                   [2]  重复成员、ISS-10-02
│   └── runtime/                        [0]
│
├── 10.4_Interface_Properties/          [9 用例]  接口属性
│   ├── compile-pass/                   [7]  readonly/rw、getter/setter、可选
│   ├── compile-fail/                   [2]  getter-only 写、setter-only 读
│   └── runtime/                        [0]
│
├── 10.5_Interface_Methods/             [10 用例] 接口方法
│   ├── compile-pass/                   [6]  抽象方法、类实现接口
│   ├── compile-fail/                   [2]  缺失实现、签名不匹配
│   └── runtime/                        [2]  ★ 唯一有 runtime 的子节
│
└── 10.6_Interface_Inheritance/         [6 用例]  接口继承
    ├── compile-pass/                   [4]  继承链、菱形、extends+implements
    ├── compile-fail/                   [2]  继承属性读写约束
    └── runtime/                        [0]
```

## 统计

| 子节 | PASS | FAIL | RT | 合计 |
|------|:--:|:--:|:--:|:--:|
| 10.1 Interface Declarations | 4 | 0 | 0 | 4 |
| 10.2 Superinterfaces | 3 | 4 | 0 | 7 |
| 10.3 Interface Members | 3 | 2 | 0 | 5 |
| 10.4 Interface Properties | 7 | 2 | 0 | 9 |
| 10.5 Interface Methods | 6 | 2 | 2 | 10 |
| 10.6 Interface Inheritance | 4 | 2 | 0 | 6 |
| **总计** | **27** | **12** | **2** | **41** |

## 已知信息

- **2 个 GAP**：Object 方法冲突规则（§10.3）编译器未实现
- **命名前缀**：全部 `ITF_` 前缀
- ⚠️ **GAP 注意**：`ITF_10_03_011_PASS_OBJECT_METHOD_CLASH.ets` 位于 compile-pass/，但规范应 FAIL（ISS-10-01：编译器未实现 §10.3 Object 方法冲突检测）
- **空文件/异常目录**：无
- **runtime**：仅 §10.5 有 2 个 runtime 用例
- **各子目录**含 4 份子报告
