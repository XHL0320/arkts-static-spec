...
**规范依据：**
- ArkTS spec §2.9.9: 使用 `null` 作为空值字面量
...
**规范依据：**
- ArkTS spec §2.9.9: `null` 是 `null` 类型的唯一字面量
...
**规范依据：**
- ArkTS spec §2.9.9: `typeof null` 返回 `"object"`
- ECMAScript: 历史遗留行为
- Java/Swift: 不支持 typeof 运算符
...
2. **运行时语义完全一致**：null 值、比较、类型检查结果完全一致
3. **4 项语言设计差异已记录**：空值字面量、类型系统、typeof 结果、比较运算符差异
