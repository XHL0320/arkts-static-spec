...
**前缀：** `STM_`
**主章节号：** 08
**覆盖汇总：** 363 cases (161P + 84F + 118R)，全部通过，执行日期 2026-06-19

---

## 子章节清单与测试策略

### 8.1 Normal and Abrupt Statement Execution `[14: 6P+3F+5R]`
...
### 8.2 Expression Statements `[17: 8P+4F+5R]`
...
### 8.3 Block `[15: 6P+4F+5R]`
- **定义：** `{ statement* }` 顺序执行，void 函数体可不含 return
- **已知问题：** STM-I1 — Block 内 type declaration，spec 措辞（"except type declarations, are executed"）暗示允许但 es2panda 拒绝 interface/type alias 声明（ESY0040）。**当前状态：待澄清 spec 措辞或编译器行为。**
- 测试点：
...
### 8.4 Constant or Variable Declarations `[23: 10P+8F+5R]`
...
### 8.5 if Statements `[26: 16P+2F+8R]` **Extended Conditional Expressions**
...
### 8.6 Loop Statements `[20: 8P+7F+5R]`
- **定义：** 循环语句的公共规则（标签、label 作用域）
- **已知问题：** STM-I2 — Loop label 未被使用，spec 要求 compile-time error 但 es2panda 未检查此约束。label 在 lambda 内使用的检查正常，但"label 完全未使用"不报错。**当前状态：编译器遗漏 spec 检查项。**
- 测试点：
...
### 8.7 while/do Statements `[25: 16P+2F+7R]` **Extended Conditional Expressions**
...
### 8.8 for Statements `[22: 13P+3F+6R]` **Extended Conditional Expressions**
...
### 8.9 for-of Statements `[23: 10P+6F+7R]`
...
### 8.10 break Statements `[19: 7P+6F+6R]`
...
### 8.11 continue Statements `[18: 6P+7F+5R]`
...
### 8.12 return Statements `[24: 10P+6F+8R]`
...
### 8.13 switch Statements `[26: 12P+4F+10R]`
...
### 8.14 throw Statements `[19: 7P+6F+6R]`
...
### 8.15 try Statements `[22: 7P+6F+9R]`
...
### 8.15.1 catch Clause `[17: 7P+4F+6R]`
...
- 测试点：
  ...
  - FAIL (4): catch(e: Error) 类型标注（ESE33781 — 已废弃）、catch 参数类型标注为其他类型、catch 无参数、catch 参数名与外层变量冲突
...
### 8.15.2 finally Clause `[18: 8P+3F+7R]`
...
### 8.15.3 try Statement Execution `[15: 4P+3F+8R]`
...
| ID | 问题 | 严重性 | 章节 | 当前状态 |
|----|------|--------|------|---------|
| STM-I1 | Block 内 type declaration — spec 措辞（"except type declarations, are executed"）暗示允许，但 es2panda 拒绝 block 内 interface/type alias 声明（ESY0040）。Java/Swift/TypeScript 均允许。 | HIGH | 8.3 | 待澄清 spec 措辞或编译器行为 |
| STM-I2 | Loop label 未被使用 — spec 明确要求 "A compile-time error occurs if the label identifier is not used within loopStatement"，但 es2panda 未检查此约束（仅检查了 "label in lambda" 的约束）。 | MEDIUM | 8.6 | 编译器遗漏 spec 检查项 |

## 需要特别注意的 ArkTS 约束

- 禁止嵌套函数声明 → 提取到顶层
- 禁止局部 class 声明 → 提取到顶层
- 禁止局部 type alias 声明 → 提取到顶层（与 STM-I1 相关：block 内同样禁止 type declaration）
...
- 逗号表达式仅限 for 循环的 init 和 update 部分，其他地方使用产生 ESY133681
