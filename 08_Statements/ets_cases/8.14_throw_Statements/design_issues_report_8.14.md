...
**评价：** 这是合理的安全设计，缩小了 throw 的表达范围，但增强了类型安全性。与 Java 的受检异常设计不同，ArkTS 不区分受检/非受检异常。新增用例（012/013/019）进一步验证了 ArkTS 对更多非 Error 类型（number、boolean、普通对象）的编译期拒绝，约束覆盖全面。
