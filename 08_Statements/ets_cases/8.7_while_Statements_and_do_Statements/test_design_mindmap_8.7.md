...
#### 1.1 基本 while 循环 (STM_08_07_001_PASS_basic_while)
...
#### 1.7 do-while 条件为 string 字面量 (STM_08_07_007_PASS_do_while_condition_string_extended)
...
#### 1.8 while 条件为 number 变量 (STM_08_07_008_PASS_while_condition_non_bool_extended)
...
#### 1.9 while 内嵌 if 中使用 break (STM_08_07_009_PASS_while_with_break_in_nested_if)
- 测试点: while 循环体内嵌套 if 语句中使用 break
- 覆盖: `if (i == 5) { break; }` 条件 break; `if (count >= 3) { break; }` 无限循环中 break
- 验证: if 内 break 可提前退出 while 循环

#### 1.10 do-while 使用 continue (STM_08_07_010_PASS_do_while_with_continue)
...
#### 1.11 while 循环体包含 try-catch (STM_08_07_011_PASS_while_body_try_catch)
...
#### 1.15 while 条件为 enum 类型 (STM_08_07_015_PASS_while_condition_enum_extended)
...
#### 2.1 while 缺少括号 (STM_08_07_009_FAIL_while_missing_parentheses)
...
#### 2.2 do-while 缺少 while 关键字 (STM_08_07_010_FAIL_do_while_missing_while)
...
#### 3.1 while(false) 循环体不执行 (STM_08_07_009_RUNTIME_while_not_executed)
- 测试点: while 条件初始为 false 时循环体一次也不执行
- 验证: `while (false)` 后 count 仍为 0; `while (x > 0)` 且 x=0 时 x 未被修改
- 断言: `if (count != 0)` 和 `if (x != 0)` 抛出错误

#### 3.2 do-while(false) 至少执行一次 (STM_08_07_010_RUNTIME_do_while_executed_once)
...
#### 3.3 while 与 do-while 行为对比 (STM_08_07_011_RUNTIME_while_vs_do_while)
...
#### 3.4 do-while 中 continue 跳转到条件检查 (STM_08_07_012_RUNTIME_do_while_continue_to_condition)
...
#### 3.5 while 内嵌 if 使用 break 提前退出 (STM_08_07_013_RUNTIME_while_break_in_nested_if)
...
#### 3.6 do-while 体内 break 至少执行一次循环体 (STM_08_07_016_RUNTIME_do_while_break_at_least_once)
...
- 断言: 4 组独立断言, 全面验证 at-least-once 语义

#### 3.7 while 零次迭代全方位验证 (STM_08_07_017_RUNTIME_while_zero_iterations)
