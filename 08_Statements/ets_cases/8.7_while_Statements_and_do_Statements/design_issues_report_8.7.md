...
- ArkTS 允许 Extended Conditional Expressions 类型（number/string/bigint/enum），但对 class 实例、数组、对象字面量等其他类型的条件行为未在 compile-fail 测试中覆盖。
...
3. **语义区分**：while 先判断后执行（条件 false 时执行 0 次），do-while 先执行后判断（条件 false 时至少执行 1 次），运行时测试全面验证。
