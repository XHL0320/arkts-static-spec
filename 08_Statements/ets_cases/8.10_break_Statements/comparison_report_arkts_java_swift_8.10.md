...
| 语言 | 定位 | 哲学 |
|------|------|------|
| ArkTS | 用于循环（while/do/for/for-of）和 switch 的控制转移语句，标签仅限循环或 switch 封闭语句 | 保守、安全：标签作用域严格限定，编译期全面检查 break 合法性 |
...
1. **break 语句是控制流中最一致、最成熟的设计之一**：三语言在 break 的行为上高度一致，开发者从任一语言迁移到其他语言时几乎无需调整心智模型。
