...
- const 复杂初始化表达式: 算术运算、三元表达式、逻辑运算、字符串拼接、数组字面量、引用其他 const
...
| 014 | const 复杂初始化表达式 | const 初始化支持算术运算(1+2\*3)、三元(true?100:0)、逻辑(true&&false)、字符串拼接("Hello"+" World")、数组字面量、引用其他 const |
