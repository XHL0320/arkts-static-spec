...
8.15 章节用例 100% 通过（22/22），所有行为均符合 ArkTS 规范 §8.15 — try 语句。未触发任何编译器异常或语义反直觉行为。
...
ArkTS 在 try/catch/finally 块内全面禁止局部 class、局部 type alias 和嵌套 function 声明（用例 007-009）。这与 Java 和 Swift 的行为显著不同。
...
**评价：** 这是 ArkTS 全局语言限制（非 try 语句特有），在 Block（8.3）、if（8.5）、循环（8.6-8.9）等其他语句块的 compile-fail 用例中同样存在。对于从 Java/Swift 迁移的开发者而言，在 try/catch/finally 块中无法声明局部辅助类型/函数可能带来迁移摩擦。
...
|------------|---------|---------|------------|------|
| STM-I1 | Block 内 type 声明 spec/impl 不一致 | 8.3 | **否** | 8.15 的 try/catch/finally 块内局部声明受 ArkTS 全局限制约束（禁止局部 class/type alias/nested function），编译器行为与 spec 一致，无 spec/impl 不匹配。8.3 的 STM-I1 特指 §8.3 "except type declarations, are executed" 措辞与编译器实际拒绝行为的矛盾，8.15 不存在类似矛盾。 |
| STM-I2 | 标签未引用不强制报错 | 8.6 | **否** | 8.15 不涉及循环标签定义与引用规则。 |
...
**总体结论：** 未发现需要修改规范或实现的设计问题。22 个测试用例首次执行 100% 通过。ArkTS try 语句的简化设计是其明确的语言设计哲学，不构成缺陷。
