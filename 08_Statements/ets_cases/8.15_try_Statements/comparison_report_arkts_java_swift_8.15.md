...
- `defer` 比 `finally` 更灵活：可以在作用域中的任意位置出现多个 `defer` 块（它们按注册的逆序执行）。`finally` 则与特定的 `try` 块绑定。
...
4. **最具影响力的差异**是 ArkTS 对 try/catch/finally 块内局部声明（类、类型别名、嵌套函数）的限制。Java 或 Swift 开发者可能会预期这些声明被允许，因为两种语言都支持它们。这是 ArkTS 为简化而做出的刻意设计选择，而非缺陷。
