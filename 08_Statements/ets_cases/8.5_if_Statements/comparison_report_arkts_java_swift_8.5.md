...
| Swift 5.x | Apple 生态现代化语言 | 极致安全设计，强制大括号消除歧义，支持条件表达式和可选值绑定 |
...
**分析：** 三种语言均在编译期拒绝非布尔类型的条件。这与 C/C++ 和 JavaScript 形成鲜明对比，后者会将整数和其他类型隐式转换为布尔值。这是 ArkTS、Java 和 Swift 共同采用的一种刻意而为的安全设计。
...
**分析：** 此用例揭示了一个显著差异。Swift 要求所有 if/else 体必须使用大括号，因此永远不会出现作用域歧义。ArkTS 和 Java 则允许单条语句省略大括号，此时不会创建块作用域。然而，ArkTS 与 Java 的不同之处在于：在 ArkTS 中，局部变量声明（`let x: int = 1`）是一条合法的语句，因此 `if (cond) let x: int = 1` 是合法的（且不创建作用域，使得 `x` 在 if 之后仍然可见）。而在 Java 中，局部变量声明不是一条语句，不能直接作为无大括号的 if 体出现。
