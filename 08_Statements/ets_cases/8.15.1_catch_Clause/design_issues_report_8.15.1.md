...
8.15.1 章节用例首次执行 100% 通过（17/17），未触发任何编译器异常或语义反直觉行为。所有测试点（基本 catch、instanceof 类型收窄、try-catch-finally、重新抛出、Error 属性访问、多子类 instanceof、错误转换、catch 返回值类型收窄）均按规范预期通过。
...
| 014 | catch 按 instanceof 类型收窄返回不同值（ZeroDivisor → -1，其他 Error → 0） | ✅ |
...
| 维度 | 评价 |
|------|------|
| 与 spec 一致性 | 100%（首次执行 17/17 全部通过） |
