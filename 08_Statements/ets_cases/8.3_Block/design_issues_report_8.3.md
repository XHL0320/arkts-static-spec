...
- 测试用例 STM_08_03_004 已重写为块变量遮蔽测试，STM_08_03_008 验证了 es2panda 当前拒绝行为
...
ArkTS 不允许在块内声明 `class`（STM_08_03_007 测试为 compile-fail）。Java（JLS 14.3）和 Swift 均允许在方法体内声明局部类。此为有意的设计约束而非缺陷——ArkTS 限制块内 class 声明以维持更简单、更可预测的类型系统，符合 ArkTS"简洁、静态安全"的设计理念。

**观察 B：块内不允许嵌套函数声明** ⭐ 有意设计

ArkTS 不允许在块内声明 `function`（STM_08_03_006 测试为 compile-fail）。这与 Java（同样不允许块内方法声明）一致，但与 Swift（允许嵌套函数）不同。此举与 ArkTS 全面禁止嵌套函数的策略一致，在任何作用域级别均不支持嵌套函数声明，块级别的限制只是这条更广泛规则的其中一种体现。
...
| 用例 ID | 行为描述 | 状态 |
|---------|---------|------|
| STM_08_03_001 | 基本块：多条语句（赋值、表达式、变量声明、函数调用、控制流）在块内顺序执行 | ✅ 通过 |
| STM_08_03_002 | 嵌套块：两级/三级嵌套、空嵌套块、跨作用域变量遮蔽 | ✅ 通过 |
| STM_08_03_003 | Void 函数体作为块并隐式返回：无 return、空 return、条件分支 return | ✅ 通过 |
| STM_08_03_004 | 块内变量遮蔽（shadowing）：内层 let 变量遮蔽外层同名变量，块结束后外层恢复（已重写，原 type declaration 版本因 STM-I1 移除） | ✅ 通过 |
| STM_08_03_005 | 空块与边界情况：完全空 `{}`、仅注释的块、仅分号的块、多个连续空块 | ✅ 通过 |
| STM_08_03_006 | 块内嵌套函数声明 → 编译失败（符合 ArkTS 无嵌套函数策略） | ✅ 通过 |
| STM_08_03_007 | 块内局部 class 声明 → 编译失败（符合 ArkTS 有意限制） | ✅ 通过 |
| STM_08_03_008 | 块内局部 type alias 声明 → 编译失败（符合 es2panda 当前实现，但与 spec 措辞存在 STM-I1 矛盾） | ⚠️ 通过（但 spec 措辞存疑） |
| STM_08_03_009 | 运行时块内语句按文本顺序依次执行，变量作用域隔离 | ✅ 通过 |
| STM_08_03_010 | 运行时块内 throw 异常导致后续语句不执行，异常被 try-catch 捕获 | ✅ 通过 |
| STM_08_03_011 | 块作用域变量生命周期：块结束后同名 var/let 变量可重新声明 | ✅ 通过 |
| STM_08_03_012 | 块外访问块内 let/const 变量 → 编译失败（块作用域语义正确） | ✅ 通过 |
| STM_08_03_013 | 运行时 4 层嵌套块变量遮蔽与逐层恢复 | ✅ 通过 |
| STM_08_03_014 | 运行时 throw 从嵌套块中间退出，跨块边界传播到 catch | ✅ 通过 |
| STM_08_03_015 | 运行时 void 函数中深层嵌套块内 return 提前终止函数执行 | ✅ 通过 |
