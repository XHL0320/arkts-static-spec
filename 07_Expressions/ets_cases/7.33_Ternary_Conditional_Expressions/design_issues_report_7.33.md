# 7.33 三元条件表达式 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.33

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.33 三元条件表达式 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：

| ID | 差异描述 | 分类 |
|----|---------|------|
| D-7.33-01 | 三元条件接受非boolean类型(int)作condition（Java/Swift均要求boolean/Bool） | 与业界静态语言差异点 |
...
- **Spec规范约束**：三元条件（condition?whenTrue:whenFalse）：右结合，编译期已知condition时类型收窄，未知时Union类型。运行时仅评估一个分支
...
| Spec一致性 | ✅ 与arktsspecification.md §7.33一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
