# 7.32.2 复合赋值运算符 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.32.2

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.32.2 复合赋值运算符 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：

| ID | 差异描述 | 分类 |
|----|---------|------|
| C-7.32-01 | &&= / ||= 复合赋值es2panda暂不支持（报Unexpected token） | 编译器实现待完善 |
...
- **Spec规范约束**：复合赋值（+= -= *= /= %= <<= >>= >>>= &= |= ^= &&= ||= ??=）：等价于lhs=(lhs op rhs) as T，lhs只求值一次。??=仅在左侧null/undefined时赋值
...
| Spec一致性 | ✅ 与arktsspecification.md §7.32.2一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
