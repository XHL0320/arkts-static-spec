# 7.35.2 Lambda体 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.35.2

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.35.2 Lambda体 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：Lambda体：单表达式（隐式return）或块体。捕获外部变量/this（引用非拷贝）。非void无return→compile-time error。void调用表达式体等价于{expression}
...
| Spec一致性 | ✅ 与arktsspecification.md §7.35.2一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
