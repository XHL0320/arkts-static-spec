...
经 es2panda + ark VM 实测，以下行为与 ArkTS spec §7.32 一致：

| 行为 | 验证方式 | 结果 |
|------|---------|------|
| 赋值表达式，=为简单赋值，复合赋值（+= -= *= /= %= <<= >>= >>>= &= |= ^= &&= ||= ??=）等价于lhs=(lhs op rhs) as T | 10 compile-pass + 10 compile-fail + 10 runtime | ✅ 全部通过 |
...
| Spec 一致性 | ✅ 与 arktsspecification.md §7.32 一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
