# 7.29 位运算与逻辑表达式 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.29

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.29 位运算与逻辑表达式 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：位运算和逻辑表达式（& ^ |），优先级& > ^ > |，操作数必须同为数值类型或同为boolean类型，否则compile-time error
...
| Spec一致性 | ✅ 与arktsspecification.md §7.29一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
