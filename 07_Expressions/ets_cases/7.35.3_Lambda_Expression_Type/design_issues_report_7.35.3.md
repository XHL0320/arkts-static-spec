# 7.35.3 Lambda表达式类型 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.35.3

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.35.3 Lambda表达式类型 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：Lambda表达式类型为函数类型（参数类型→返回类型）。返回类型可从lambda体推断，因此返回类型标注可省略
...
| Spec一致性 | ✅ 与arktsspecification.md §7.35.3一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
