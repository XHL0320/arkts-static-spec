# 7.26 移位表达式 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.26

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.26 移位表达式 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：

| ID | 差异描述 | 分类 |
|----|---------|------|
| D-7.26-01 | nullish类型(int|null)作为移位操作数未被es2panda拒绝 | 与业界静态语言差异点 |
...
- **Spec规范约束**：移位表达式（<< >> >>>），操作数必须为数值类型或bigint，double/float操作数先截断为long/int，int用5-bit/long用6-bit掩码指定移位距离
...
| Spec一致性 | ✅ 与arktsspecification.md §7.26一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
