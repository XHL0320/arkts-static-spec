# 7.32.1 简单赋值运算符 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.32.1

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.32.1 简单赋值运算符 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：简单赋值（=）：类型不兼容→compile-time error，readonly赋值→compile-time error。运行时数组越界→RangeError，FixedArray类型不匹配→ArrayStoreError
...
| Spec一致性 | ✅ 与arktsspecification.md §7.32.1一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
