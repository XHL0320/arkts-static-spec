# 7.34 字符串插值表达式 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.34

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.34 字符串插值表达式 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：字符串插值（反引号+\${expr}）：嵌入表达式隐式转string（同字符串拼接转换规则），支持嵌套多行字符串
...
| Spec一致性 | ✅ 与arktsspecification.md §7.34一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
