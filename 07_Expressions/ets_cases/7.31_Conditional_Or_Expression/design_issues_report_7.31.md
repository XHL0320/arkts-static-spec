# 7.31 条件或表达式 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.31

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.31 条件或表达式 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：条件或（||）：短路求值——左侧为true时跳过右侧，左→右结合，完全满足结合律。操作数必须为boolean或Extended Conditional Expressions类型
...
| Spec一致性 | ✅ 与arktsspecification.md §7.31一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
...
- 测试执行报告：[test_report_7.31.md](test_report_7.31.md)
- 跨语言对比：[comparison_report_arkts_java_swift.md](comparison_report_arkts_java_swift.md)
- 测试设计：[test_design_mindmap_7.31.md](test_design_mindmap_7.31.md)
