# 7.32.3 左值表达式 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.32.3

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.32.3 左值表达式 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：左值表达式：变量、参数（非this）、字段/setter、数组元素、record元素。禁止chaining operator ?.和readonly实体作为左值
...
| Spec一致性 | ✅ 与arktsspecification.md §7.32.3一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
