# 7.27.3 字符串关系运算符 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.27.3

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.27.3 字符串关系运算符 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：字符串字典序(lexicographic)比较：< <= > >= 基于码元顺序，空字符串小于任何非空字符串
...
| Spec一致性 | ✅ 与arktsspecification.md §7.27.3一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
