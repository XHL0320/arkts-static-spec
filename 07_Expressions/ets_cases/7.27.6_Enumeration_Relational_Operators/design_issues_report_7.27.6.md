# 7.27.6 枚举关系运算符 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.27.6

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.27.6 枚举关系运算符 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：

| ID | 差异描述 | 分类 |
|----|---------|------|
| D-7.27-01 | 枚举类型与int关系比较未被es2panda拒绝（Spec要求同枚举类型） | 与业界静态语言差异点 |
...
- **Spec规范约束**：同枚举类型比较：int基类型→数值比较，string基类型→字符串比较。不同枚举类型或枚举vs其他类型→compile-time error
...
| Spec一致性 | ✅ 与arktsspecification.md §7.27.6一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
