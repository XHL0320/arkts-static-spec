# 7.29.1 整数位运算符 - ArkTS与Java/Swift/TS行为差异及规范一致性报告
...
**Spec依据：** arktsspecification.md §7.29.1

---

## 一、与业界静态语言的差异点

本章节验证了 ArkTS §7.29.1 整数位运算符 规范约束。以下为实测中发现的 **ArkTS与Java/Swift/TS行为差异**，按 C类（编译器实现待完善）/ D类（Spec与实现不一致）分类：
...
- **Spec规范约束**：整数位运算：类型提升规则（byte/short→int, double/float截断为long/int），bigint不允许与numeric混合，结果类型为提升后的操作数类型
...
| Spec一致性 | ✅ 与arktsspecification.md §7.29.1一致 | ✅ JLS SE21 | ✅ Swift 5.10 | N/A |
...
- 测试执行报告：[test_report_7.29.1.md](test_report_7.29.1.md)
- 跨语言对比：[comparison_report_arkts_java_swift.md](comparison_report_arkts_java_swift.md)
- 测试设计：[test_design_mindmap_7.29.1.md](test_design_mindmap_7.29.1.md)
