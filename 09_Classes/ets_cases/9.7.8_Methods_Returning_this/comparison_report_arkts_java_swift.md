...
2. **编译期 `return this` 约束是 ArkTS 独有的安全保证**：Java/Swift 的 builder 方法可以返回 `null`、`new` 对象或其他类型，编译器完全不检查。ArkTS 在编译期确保声明为 `this` 返回类型的方法只能返回 `this` 或另一个 `this`-returning 方法的结果。这种约束消除了 builder 模式下最常见的误用。
