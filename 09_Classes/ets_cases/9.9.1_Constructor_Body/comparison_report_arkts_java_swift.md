...
- ⭐ **Swift 无继承默认值**：Swift 中类（class）默认不支持继承——必须显式继承自 NSObject 或其他 class。而 ArkTS 和 Java 的类默认支持单继承（Object/Any 为根）。
...
| **字段初始化顺序保障** | ⭐⭐⭐⭐⭐ 超级验证 GFGcPfPcCfCc | ⭐⭐⭐⭐ JLS §12.5 隐式 | ⭐⭐⭐⭐⭐ 两阶段 Phase 1 强制 |
...
| **文档/规范清晰度** | ⭐⭐⭐⭐⭐ spec 五步顺序清晰 | ⭐⭐⭐⭐ JLS 全面但分散 | ⭐⭐⭐⭐⭐ 官方文档清晰 |
