...
## 概述
本节定义 ArkTS 中类的**实例方法 (Instance Method)**：声明在 class body 中、不带 `static` 修饰符的方法。实例方法通过对象实例调用，`this` 在方法体内指向当前实例，可访问实例字段 (`this.field`)、调用其他实例方法 (`this.method()`)、实现接口方法 (`implements`)、重写超类方法 (`override`)，以及通过 `super.method()` 调用父类实现。ArkTS **不支持方法重载 (overloading)** — 同一类中不允许存在名称相同且参数类型列表相同的方法签名，即使返回类型不同也不允许。
