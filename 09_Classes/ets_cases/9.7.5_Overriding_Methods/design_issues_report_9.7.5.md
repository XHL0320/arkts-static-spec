...
### 问题 CLS-I2：重写方法默认参数值不匹配未被编译器拒绝 ⭐⭐ MEDIUM
...
| Swift | ✅ 必须使用 override |
| Kotlin | ✅ 必须使用 override |
| C++ | ✅ 必须使用 override（C++11+） |

**评价：** ArkTS 延续了 TypeScript 的灵活风格。`override` 关键字为可选修饰符，省略时仅失去编译器的重写正确性检查。这与 Swift/Kotlin 的强制安全策略不同。建议团队内部编码规范强制要求使用 `override` 关键字以提升可维护性。
...
**无重现。** 9.7.5 章节首次执行 100% 通过（但发现上述 CLS-I2 设计问题）。
...
|-------|------|---------|
| ⭐⭐⭐ HIGH | 0 | - |
| ⭐⭐ MEDIUM | 1 | 问题 CLS-I2：重写方法默认参数值不匹配未被编译器拒绝 |
| ⭐ LOW | 0 | - |
| 设计观察（非问题）| 2 | A（override 可选）、B（协变返回类型） |
...
| 与 Swift/Kotlin 对比 | 安全检查弱于 Swift/Kotlin（override 可选 + 无默认参数检查） |
