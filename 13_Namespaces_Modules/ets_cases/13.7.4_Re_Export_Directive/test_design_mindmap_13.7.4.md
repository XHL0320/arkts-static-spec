...
spec §13.7.4：re-export directive 将其他模块的导出重新导出。包括 `export * from "path"` 和 `export {d1} from "path"`。re-export引用当前文件 → compile-time error。re-export实体不可区分 → compile-time error。
