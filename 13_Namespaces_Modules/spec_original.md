...
| 13.4.2 | Default Import Binding | 单标识符导入或 `{default as Name}`; 用其他形式导入 default 导出 → 编译错误 |
...
| 13.6 | Exported Declarations | export 修饰符使声明可被其他模块 import；导出名与另一导出名重复 → 编译错误；仅一个 default export；default export expression 创建匿名常量变量；导出实体类型必须显式设置（函数/变量/常量/类成员）；导出实体使用未导出类型 → 编译错误（含 extends/implements、泛型约束/默认值、type alias、annotation） |
