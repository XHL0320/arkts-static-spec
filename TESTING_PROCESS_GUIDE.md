...
每个用例对照点必须包含如下表格，列格式为：

```
| # | 用例 | ArkTS | Java | Swift |
|---|------|-------|------|-------|
| 001 | 用例描述 | ✅ compile-pass / ✅ compile-fail / ✅ runtime / ⚠️ / ❌ | 同左 | 同左 |
```

**列说明：**
- `#`：用例编号，与 ets_cases 中的用例编号对应
- `用例`：用例简短描述（与测试设计思维导图对应）
- `ArkTS`：以 `✅ compile-pass`、`✅ compile-fail`、`✅ runtime`、`⚠️`（部分通过/行为异常）、`❌`（不支持/失败）标注
- `Java`：同上，或 `N/A`（无对应概念）标注
- `Swift`：同上，或 `N/A`（无对应概念）标注

**关键差异用例必须追加"关键差异详解"子表格：**
