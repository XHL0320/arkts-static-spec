...
|---|------|--------|---------|--------|--------|
| D-4.01-01 | NAM_04_01_015_FAIL_TYPE_KEYWORD | Spec 未完整列举保留名 | compile-time error | 编译器保留额外名称 | D类-Spec不一致 |
| D-4.06-01 | NAM_04_06_05_002_FAIL_CONST_OBJECT_LITERAL_INFER | `int` 作为隐式推断类型未正式定义 | Spec 明确定义 | Spec 注释使用但未定义 | D类-Spec不一致 |
...
- **复现用例 ID：** NAM_04_01_015_FAIL_TYPE_KEYWORD
- **实测结果：**
  ```typescript
...
- **复现用例 ID：** NAM_04_06_05_002_FAIL_CONST_OBJECT_LITERAL_INFER
- **跨语言对比：**
