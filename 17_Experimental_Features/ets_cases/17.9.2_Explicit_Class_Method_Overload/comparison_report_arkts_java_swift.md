...
2. **修饰符一致性强制验证是 ArkTS 的独特安全设计。** static/instance 一致性、async 一致性、访问修饰符一致性都是编译期强制验证，Java/Swift 均无此类约束。这防止了重载集合内部的语义不一致。
...
1. 当前设计在 API 设计灵活性和类型安全之间取得了良好平衡。`overload` 声明 + 不同名方法绑定的模式是优秀的 API 设计工具。
