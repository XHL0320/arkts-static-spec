...
#### 与 Kotlin receiver lambda 的对比

```kotlin
// Kotlin: 支持隐式 this（最接近 ArkTS spec 描述的理想行为）
class Counter { var count: Int = 0 }
val inc: Counter.() -> Unit = {
    count++     // ✅ 隐式 this（Kotlin 支持）
    this.count++ // ✅ 也可以显式
}
```

Kotlin 的 receiver lambda 完全支持隐式 this——`count` 自动解析为 `this.count`。当有歧义时，Kotlin 也要求显式 `this@Counter.count`。这与 ArkTS spec 描述的行为完全一致。
...
1. **ArkTS spec 描述的隐式 this 是一个便捷特性**，允许 receiver lambda body 中省略 `this.` 前缀——设计上与 Kotlin receiver lambda 一致。
...
4. **Kotlin 是当前唯一完整实现 receiver lambda 隐式 this 的主流语言**，可作为 es2panda 未来实现的参考。
