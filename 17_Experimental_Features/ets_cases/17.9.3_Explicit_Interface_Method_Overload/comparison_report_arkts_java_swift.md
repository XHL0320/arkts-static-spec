...
1. **ArkTS 在接口中支持显式 overload 声明是独特设计。** 没有任何对比语言在接口/协议级别提供重载绑定这一概念。Java 接口的方法各自独立（即使同名也是独立的签名重载），Swift protocol 也是如此。

2. **实现类可以 override 接口 overload 并添加新方法，这是强大的 API 扩展机制。** 接口定义了最小重载集合，实现类可以在不破坏接口契约的前提下扩展重载能力。
...
1. 接口 overload + 实现类可选 override 的机制设计良好，建议在 spec 中增加更多使用场景和最佳实践。
