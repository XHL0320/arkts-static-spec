...
**说明：** Java 完全无 receiver 函数等价语法。要给已存在类添加方法，只能通过继承或包装（Decorator 模式）。Swift 的 `extension` 可以向已有类型添加方法，但语法完全不同——不需要 `this: Type` 参数，直接在 extension 块内定义方法即可。ArkTS 的 receiver 函数是独特的顶层扩展机制：函数定义在类外部，通过 `this: Type` 第一参数声明其接收者，进而获得方法调用语法糖。
