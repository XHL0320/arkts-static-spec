...
### 差异 A：显式 overload 声明是 ArkTS 独特设计，Java/Swift 无对应语法
...
ArkTS 要求所有重载函数必须在同一模块/命名空间作用域内，跨作用域引用会产生编译错误。Java 的方法重载天然受限于类作用域，Swift 的方法重载受限于类型作用域。ArkTS 的模块作用域重载是独特的顶层设计。
...
| 条目 | 分类 |
|------|------|
| 差异 A：显式 overload 声明是独特设计 | 符合 ArkTS spec 的语言设计差异 |
