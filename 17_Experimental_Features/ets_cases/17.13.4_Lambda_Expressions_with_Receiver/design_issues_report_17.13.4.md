...
**说明：** Java 的 lambda 表达式完全不具备 receiver 概念——`(String s) -> s.length()` 中的第一个参数只是普通参数。Swift 闭包可以命名参数为 `self`，但没有语言级 receiver 语义支持。Kotlin 的 "Function Literals with Receiver"（`val greet: Greeter.() -> Unit = { ... }`）是最接近 ArkTS 的设计，但两者语法差异显著。
